...
(function() {
  ...
  const Auth = {
    ...
    async signInWithPassword({ email, password }) {
      if (!API_URL) return { error: new Error('API URL not configured'), data: null };
      try {
        ...
        if (!res.ok) {
          return { error: new Error(json.error || 'Login failed'), data: null };
        }
    ...
    async signUp({ email, password, options = {} }) {
      ...
      try {
        ...
        const json = await res.json().catch(() => ({}));
        if (!res.ok) {
          return { error: new Error(json.error || 'Signup failed'), data: null };
        }
        setToken(json.token);
        return {
          data: {
    ...
    async resend({ type, email }) {
      return { error: new Error('Email resend is not implemented'), data: null };
    },
    ...
    async mfaListFactors() {
      ...
      try {
        ...
        const json = await res.json().catch(() => ({}));
        if (!res.ok) {
          return { error: new Error(json.error || 'Failed to list factors'), data: null };
        }
        return { data: { totp: json.totp || [] }, error: null };
      } catch (e) {
        return { error: e, data: null };
      }
    },
    ...
    async mfaGetAuthenticatorAssuranceLevel() {
      ...
      try {
        ...
        const json = await res.json().catch(() => ({}));
        if (!res.ok) {
          return { error: new Error(json.error || 'Failed to get assurance level'), data: null };
        }
        return { data: { currentLevel: json.currentLevel, nextLevel: json.nextLevel }, error: null };
      } catch (e) {
        return { error: e, data: null };
      }
    },
    ...
    async refreshSession() {
      ...
      try {
        ...
        const json = await res.json().catch(() => ({}));
        if (!res.ok) {
          return { error: new Error(json.error || 'Refresh failed'), data: null };
        }
        if (json.token) setToken(json.token);
        return { data: { session: { access_token: json.token } }, error: null };
      } catch (e) {
        return { error: e, data: null };
      }
    },
