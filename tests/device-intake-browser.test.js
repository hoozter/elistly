...
(async () => {
  ...
  try {
    ...
    assert.match(await settings.textContent(), /Windows device collector.*workspace-bound.*no automatic expiry/is);
    assert.equal(await settings.getByRole('button', { name: 'Create', exact: true }).count(), 1);
    assert.doesNotMatch(await settings.textContent(), /24.hour|24 hours/i);
    assert.match(await settings.textContent(), /choose an expiry date and time.*revoke it manually/is);
    ...
    assert.match(registrationScript, /Win32_LogicalDisk.*Get-Tpm.*Confirm-SecureBootUEFI.*Get-BitLockerVolume/is);
    assert.match(registrationScript, /designCapacityMWh.*fullChargeCapacityMWh.*lastInteractiveUser.*availability/is);
    assert.match(registrationScript, /Select-Object -First 1/, 'optional CIM queries must not index a null result');
    assert.match(registrationScript, /\[datetime\]\$Value/, 'CIM DateTime values must be converted directly');
    assert.match(registrationScript, /CurrentVersion.*DisplayVersion/, 'Windows release must come from the local registry');
    assert.match(registrationScript, /Win32_Tpm.*SpecVersion/, 'TPM version must be the TPM specification version');
    assert.match(registrationScript, /Get-NetAdapter.*Get-NetIPAddress.*networkAdapters/, 'only local adapter addresses may be collected');
    assert.match(registrationScript, /observation = 'current interactive session'/i, 'the username is a current-session observation, not logon history');
    assert.match(registrationScript, /\[switch\]\$Preview.*Preview only: no data was transmitted/is);
    assert.doesNotMatch(registrationScript, /RecoveryPassword|recovery key|Get-ScheduledTask|Register-ScheduledTask|Set-ExecutionPolicy|Bypass/i);
