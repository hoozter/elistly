...
async function testActivationRetiresOnlyPriorElistlyShells() {
  const harness = loadServiceWorker({ fetch: async () => response(''), keys: ['elistly-shell-v10', 'elistly-shell-v11', 'unrelated-app-v1'] });
  let completed;
  harness.listeners.activate({ waitUntil: promise => { completed = promise; } });
  await completed;
  assert.deepEqual(harness.deleted, ['elistly-shell-v10'], 'activation must retire only prior Elistly shell caches');
}
