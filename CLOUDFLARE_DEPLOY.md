...
4. **Environment variables** (Settings → Environment variables): add for **Production** (and optionally Preview):
   - `ELISTLY_BACKEND_PROVIDER` = `supabase`
   - `ELISTLY_BACKEND_URL` = your Supabase project URL (e.g. `https://xxxx.supabase.co`)
   - `ELISTLY_PUBLIC_KEY` = your Supabase anon public key (long JWT)
...
4. **Variables and Secrets:** In the Worker → **Settings** → **Variables and Secrets**, add `ELISTLY_BACKEND_PROVIDER`, `ELISTLY_BACKEND_URL`, `ELISTLY_PUBLIC_KEY`, and `ELISTLY_SERVICE_ROLE_KEY` as **Secret** (encrypted), not Variable (plaintext). Legacy `SUPABASE_*` names still work during migration. Secrets persist across deploys.
...
| Secrets | Pages env vars for the site (for example `ELISTLY_BACKEND_URL`, `ELISTLY_API_URL`); Worker Variables and Secrets for the API (`ELISTLY_BACKEND_URL`, `ELISTLY_SERVICE_ROLE_KEY`). |
