...
- Review stored-XSS/browser policy protection, local inventory/outbox retention, logout cleanup and shared-device exposure without discarding unsynced changes.
  - Local retention/sign-out hardening, including storage-layer cross-tab invalidation, is implemented and locally verified; [scope, residual risks and policy decisions](docs/local-retention-security.md) are recorded separately. Hosted provider-cookie acceptance and a complete cross-tab lifecycle policy remain open; this is not a deployment or shared-browser safety claim.
