...
- Review stored-XSS/browser policy protection, local inventory/outbox retention, logout cleanup and shared-device exposure without discarding unsynced changes.
  - Local retention/sign-out hardening is implemented and locally verified; [scope, residual risks and policy decisions](docs/local-retention-security.md) are recorded separately. Cross-tab isolation and hosted provider-cookie acceptance remain open; this is not a deployment or shared-browser safety claim.
